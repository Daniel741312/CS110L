...
fn main() {
    ...
    let ps_res = ps_utils::get_target(&target).expect("there being a problem calling ps or pgrep");
    if let Some(p) = ps_res {
        println!("Found pid {}", p.pid);
        p.print();
        let child_processes = ps_utils::get_child_processes(p.pid).expect("get child processes error");
